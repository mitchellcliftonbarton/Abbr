      ...
      <?php if ($group_title and $group_text and $group_link): ?>
        <a href="<?= $group_link['url'] ?>" target="<?= $group_link['target'] ?? '_self' ?>" class="group-item grid grid-cols-12 gap-8 border-b border-black px-8 pt-3 pb-6">
          <div class="group-item-title col-span-8">
            <h3 class="title text-lg text-grey-3 tracking-[-.02em] leading-none font-medium"><?= $group_title ?></h3>

            <?php if ($group_subtitle): ?>
              <div class="subtitle text-lg text-grey-2 tracking-[-.02em] leading-none font-medium w-1/2 max-w-[500px]"><?= $group_subtitle ?></div>
            <?php endif; ?>
          </div>

          <div class="col-span-4 relative">
            <div class="text text-lg text-grey-3 tracking-[-.02em] leading-none font-medium"><?= nl2br($group_text) ?></div>
